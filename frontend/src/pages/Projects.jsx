...
const [filters, setFilters] = useState({
  ...
  city: '',
  status: '',
  date: '',         // for registration_date
  expiryDate: ''    // for expiry_date
});
  ...
  if (!isAdmin && !userAccessFields.includes('projects')) {
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <div className="text-center p-8 bg-white rounded-lg shadow-md">
        <h2 className="text-xl font-semibold text-gray-800 mb-2">Access Denied</h2>
        <p className="text-gray-600">You don't have permission to access Projects.</p>
      </div>
    </div>
  );
}
...
let filtered = projects.filter(project => {
  const matchesSearch = 
    ...
    project.promoter_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  // Match exact registration date
  let matchesRegistrationDate = true;
  if (filters.date && project.registration_date) {
    const projectDate = new Date(project.registration_date).toISOString().split('T')[0];
    matchesRegistrationDate = projectDate === filters.date;
  }

  // Match exact expiry date
  let matchesExpiryDate = true;
  if (filters.expiryDate && project.expiry_date) {
    const expDate = new Date(project.expiry_date).toISOString().split('T')[0];
    matchesExpiryDate = expDate === filters.expiryDate;
  }
        ...
        <div className="  p-6 pt-0 mb-0">
          <div className="flex items-center justify-between gap-4">
            <div className='flex-1'>
              <h1 className="text-3xl font-bold text-[#2F4C92] mb-2">Projects</h1>
              <p className="text-gray-500">Manage and monitor all your projects in one place</p>
            </div>
...
{/* <div className="grid grid-cols-1 sm:grid-cols-2 gap-6"> */}
  ...
  <div>
    ...
    <input
      ...
      className="w-full p-3 rounded-lg border border-gray-300 focus:border-[#5caaab] focus:ring-2 focus:ring-[#5caaab] outline-none"
    />
  </div>
  ...
  <div>
    ...
    <input
      ...
      onChange={(e) => {
        setFilters(prev => ({ ...prev, expiryDate: e.target.value }));
        setCurrentPage(1);
      }}
                ...
                <tbody>
                  {currentProjects.map((project, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-gray-700">{project.promoter_name || 'N/A'}</td>
                      <td className="p-4">
...
<td className="p-4">
  <div className="text-gray-900">
    {project.registration_date
      ? new Date(project.registration_date).toLocaleDateString('en-US', {
          day: '2-digit',
          month: 'short',
          year: 'numeric',
        })
  ...
  <div className="text-sm text-gray-500">
    Exp:{' '}
    {project.expiry_date
      ? new Date(project.expiry_date).toLocaleDateString('en-US', {
          day: '2-digit',
          month: 'short',
          year: 'numeric',
        })
